use super::{BrokerStats, InMemoryConfig, InMemoryError};
use crate::Message;
...
impl TopicData {
    ...
    fn add_message(
        ...
        if add_metadata {
            message
                .metadata
                .insert("_sequence".to_string(), self.next_sequence.to_string());
            ...
            message
                .metadata
                .insert("_enqueued_at".to_string(), now_millis.to_string());
        }
...
impl InMemoryBroker {
        ...
        for mut message in messages {
            ...
            if self.config.maintain_order {
                message.metadata.insert(
                    "_sequence".to_string(),
                    topic_data.next_sequence.to_string(),
                );
            ...
            if self.config.enable_persistence {
                topic_data.add_message(message.clone(), self.config.max_queue_size, false)?;
    ...
    pub fn subscribe(
        &self,
        topic: &str,
    ) -> Result<mpsc::UnboundedReceiver<Message>, InMemoryError> {
        if self.is_shutdown() {
            return Err(InMemoryError::BrokerShutdown);
        }
    ...
    pub fn cleanup_idle_topics(
        &self,
        idle_threshold: Duration,
    ) -> Result<Vec<String>, InMemoryError> {
        let mut topics = self.topics.write().unwrap();
        let mut removed_topics = Vec::new();

        // Find idle topics
        let idle_topic_names: Vec<String> = topics
            .iter()
    ...
    fn estimate_memory_usage_with_lock(
        &self,
        topics: &std::collections::HashMap<String, TopicData>,
    ) -> usize {
...
impl InMemoryBroker {
    ...
    pub fn subscribe_with_ack(
        ...
        tokio::sync::mpsc::UnboundedReceiver<(
            crate::Message,
            crate::memory::ack::InMemoryAckHandle,
        )>,
        InMemoryError,
    > {
        if self.is_shutdown() {
    ...
    pub async fn ack_message(
        &self,
        _handle: &crate::memory::ack::InMemoryAckHandle,
    ) -> Result<(), InMemoryError> {
        if self.is_shutdown() {
            return Err(InMemoryError::BrokerShutdown);
        }
    ...
    pub async fn ack_batch(
        &self,
        handles: &[crate::memory::ack::InMemoryAckHandle],
    ) -> Result<(), InMemoryError> {
        if self.is_shutdown() {
            return Err(InMemoryError::BrokerShutdown);
        }
    ...
    pub async fn nack_batch(
        &self,
        _handles: &[crate::memory::ack::InMemoryAckHandle],
        requeue: bool,
    ) -> Result<(), InMemoryError> {
        if self.is_shutdown() {
            return Err(InMemoryError::BrokerShutdown);
