...
mod integration_tests {
    ...
    async fn test_complete_message_flow() {
        ...
        for expected_order in &orders {
            ...
            assert!(
                received_order.metadata.contains_key("_sequence"),
                "Missing _sequence metadata"
            );
            assert!(
                received_order.metadata.contains_key("_enqueued_at"),
                "Missing _enqueued_at metadata"
            );

            println!(
                "Processed order: {} for customer: {}",
    ...
    async fn test_multiple_topics_and_subscribers() {
        ...
        order_publisher
            .publish("orders", order.clone())
            .await
            .unwrap();
        notification_publisher
            .publish("notifications", notification.clone())
            .await
            .unwrap();
    ...
    async fn test_high_throughput_scenario() {
        ...
        for chunk in messages.chunks(batch_size) {
            publisher
                .publish("high-volume", chunk.to_vec())
                .await
                .unwrap();
        }
    ...
    async fn test_error_handling_scenarios() {
        ...
        let result = publisher
            .publish("", vec![Message::new(b"test".to_vec())])
            .await;
        assert!(matches!(
            result,
    ...
    async fn test_concurrent_operations() {
        ...
        let publish_tasks: Vec<_> = publishers
            .into_iter()
            .enumerate()
            .map(|(i, publisher)| {
                tokio::spawn(async move {
                    let messages = vec![Message::new(
                        format!("Message from publisher {}", i).into_bytes(),
                    )];
                    publisher
                        .publish("concurrent-test", messages)
                        .await
                        .unwrap();
                })
            })
        ...
        for subscriber in &mut subscribers {
            for _ in 0..5 {
                let message = timeout(Duration::from_secs(1), subscriber.receive())
                    ...
                    .unwrap();
                assert!(
                    String::from_utf8_lossy(&message.payload).contains("Message from publisher")
                );
            }
        }
    ...
    async fn test_message_metadata_preservation() {
        ...
        publisher
            .publish("metadata-test", vec![original_message.clone()])
            .await
            .unwrap();
        ...
        // Verify system metadata is present
        assert!(
            received_message.metadata.contains_key("_sequence"),
            "Missing _sequence metadata"
        );
        assert!(
            received_message.metadata.contains_key("_enqueued_at"),
            "Missing _enqueued_at metadata"
        );

        // Verify specific metadata values
        assert_eq!(
            received_message.metadata.get("content-type"),
            Some(&"application/json".to_string())
        );
        assert_eq!(
            received_message.metadata.get("priority"),
            Some(&"high".to_string())
        );
        assert_eq!(
            received_message.metadata.get("source"),
