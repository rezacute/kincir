...
impl MQTTAckSubscriber {
    ...
    pub async fn new(broker_url: &str, client_id: Option<String>) -> Result<Self, MQTTError> {
        let client_id = client_id.unwrap_or_else(|| format!("kincir-mqtt-ack-{}", Uuid::new_v4()));
    ...
    pub async fn new(broker_url: &str, client_id: Option<String>) -> Result<Self, MQTTError> {
        ...
        tokio::spawn(async move {
            Self::handle_event_loop_with_logging(
                event_loop_clone,
                message_tx,
                client_clone,
                logger_clone,
        ...
        loop {
            ...
            match event_result {
                ...
                Ok(Event::Incoming(Packet::PubAck(puback))) => {
                    logger
                        .info(&format!("Received PUBACK for packet_id: {}", puback.pkid))
                        .await;
                }
                Ok(Event::Incoming(Packet::PubRec(pubrec))) => {
                    logger
                        .info(&format!("Received PUBREC for packet_id: {}", pubrec.pkid))
                        .await;
                }
                Ok(Event::Incoming(Packet::PubComp(pubcomp))) => {
                    logger
                        .info(&format!("Received PUBCOMP for packet_id: {}", pubcomp.pkid))
                        .await;
                }
                Ok(event) => {
    ...
    pub async fn subscribe_with_qos(&self, topic: &str, qos: QoS) -> Result<(), MQTTError> {
        ...
        self.logger
            .info(&format!(
                "Subscribing to MQTT topic {} with QoS {:?}",
                topic, qos
            ))
            .await;
...
impl AckSubscriber for MQTTAckSubscriber {
    ...
    async fn subscribe(&self, topic: &str) -> Result<(), Self::Error> {
        ...
        self.subscribe_with_qos(topic, QoS::AtLeastOnce)
            .await
    ...
    async fn receive_with_ack(&mut self) -> Result<(Message, Self::AckHandle), Self::Error> {
        #[cfg(feature = "logging")]
        self.logger
            .info("Waiting to receive MQTT message with acknowledgment")
            .await;

        let mut message_rx = self.message_rx.lock().await;
        let (message, mut ack_handle) = message_rx.recv().await.ok_or_else(|| {
            Box::new(MQTTError::ReceiveError(
    ...
    async fn ack_batch(&self, handles: Vec<Self::AckHandle>) -> Result<(), Self::Error> {
        ...
        self.logger
            .info(&format!(
                "Batch acknowledging {} MQTT messages",
                handles.len()
            ))
            .await;
