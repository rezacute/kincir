...
use kincir::memory::{InMemoryAckSubscriberFixed, InMemoryBroker, InMemoryPublisher};
...
mod cross_backend_consistency_tests {
    ...
    async fn test_in_memory_acknowledgment_consistency() {
        ...
        for i in 0..5 {
            ...
            assert_eq!(
                received.metadata.get("test_type"),
    ...
    async fn test_acknowledgment_handle_properties() {
        ...
        assert!(
            !handle.message_id().is_empty(),
            "Message ID should not be empty"
        );
        assert_eq!(handle.topic(), topic, "Topic should match");
        assert_eq!(
            handle.delivery_count(),
            1,
            "Initial delivery count should be 1"
        );
        ...
        assert_eq!(
            received.payload, test_message.payload,
            "Payload should match"
        );
        assert_eq!(
            received.metadata.get("prefix"),
            Some(&"HandleProps".to_string()),
    ...
    async fn test_negative_acknowledgment_consistency() {
        ...
        subscriber
            .nack(handle, true)
            .await
            .expect("Failed to nack with requeue");
...
mod high_throughput_integration_tests {
    ...
    async fn test_high_throughput_acknowledgment_workflow() {
        ...
        println!(
            "   Publish throughput: {:.2} messages/second",
            publish_throughput
        );
        ...
        assert!(
            throughput > 50.0,
            "Acknowledgment throughput should be > 50 msg/sec, got {:.2}",
            throughput
        );
        assert!(
            avg_ack_time.as_millis() < 100,
            "Average ack time should be < 100ms, got {:?}",
            avg_ack_time
        );
        assert!(
            publish_throughput > 100.0,
            "Publish throughput should be > 100 msg/sec, got {:.2}",
            publish_throughput
        );
    ...
    async fn test_concurrent_acknowledgment_operations() {
                ...
                for _ in 0..messages_to_publish {
                    match timeout(Duration::from_secs(5), subscriber.receive_with_ack()).await {
                        Ok(Ok((message, handle))) => {
                            // Verify message
                            assert!(
                                String::from_utf8_lossy(&message.payload).contains("ConcurrentAck")
                            );
                        ...
                        Err(_) => {
                            println!(
                                "Subscriber {} timeout after {} messages",
                                sub_id, processed_count
                            );
                            break;
                        }
                    }
        ...
        for handle in subscriber_handles {
            let (sub_id, count, duration, avg_ack_time) =
                handle.await.expect("Subscriber task failed");
            total_processed += count;
            all_ack_times.push(avg_ack_time);

            println!(
                "📥 Subscriber {} processed {} messages in {:?} (avg ack: {:?})",
        ...
        println!(
            "   Expected total (broadcast): {}",
            messages_to_publish * concurrent_subscribers
        );
        println!(
            "   Processing efficiency: {:.1}%",
            (total_processed as f64 / (messages_to_publish * concurrent_subscribers) as f64)
                * 100.0
        );
        ...
        assert_eq!(
            ...
            "All messages should be processed by all concurrent subscribers in broadcast mode"
        );
...
mod end_to_end_workflow_tests {
    ...
    async fn test_complete_acknowledgment_workflow() {
        ...
        assert_eq!(
            successful_acks, 8,
            "Should have 8 successful acknowledgments"
        );
    ...
    async fn test_multi_topic_acknowledgment_workflow() {
        ...
        publisher
            .publish(topic1, messages1.clone())
            .await
            .expect("Failed to publish to topic1");
        publisher
            .publish(topic2, messages2.clone())
            .await
            .expect("Failed to publish to topic2");
        publisher
            .publish(topic3, messages3.clone())
            .await
...
mod integration_test_utilities {
    ...
    async fn test_integration_test_utilities() {
        ...
        for (i, message) in messages.iter().enumerate() {
            ...
            assert_eq!(
                message.metadata.get("test_type"),
                Some(&"integration".to_string())
            );
            assert_eq!(message.metadata.get("sequence"), Some(&i.to_string()));
            assert_eq!(
                message.metadata.get("prefix"),
                Some(&"UtilityTest".to_string())
            );
...
mod performance_validation_tests {
    ...
    async fn test_acknowledgment_latency_validation() {
        ...
        let avg_receive_latency =
            latencies.iter().map(|(r, _, _)| *r).sum::<Duration>() / latencies.len() as u32;
        let avg_ack_latency =
            latencies.iter().map(|(_, a, _)| *a).sum::<Duration>() / latencies.len() as u32;
        let avg_total_latency =
            latencies.iter().map(|(_, _, t)| *t).sum::<Duration>() / latencies.len() as u32;
        ...
        // Performance assertions
        assert!(
            avg_receive_latency.as_millis() < 50,
            "Average receive latency should be < 50ms"
        );
        assert!(
            avg_ack_latency.as_millis() < 10,
            "Average ack latency should be < 10ms"
        );
        assert!(
            avg_total_latency.as_millis() < 60,
            "Average total latency should be < 60ms"
        );
        assert!(
            max_total_latency.as_millis() < 200,
            "Max total latency should be < 200ms"
        );
    ...
    async fn test_memory_usage_validation() {
        ...
        println!(
            "   Queue increase: {} messages",
            after_publish_queued - initial_queued
        );
        ...
        println!(
            ...
            after_publish_queued - final_queued
        );

        // Message processing assertions
        assert!(
            after_publish_queued > initial_queued,
            "Queue should increase after publishing"
        );
        assert!(
            final_queued <= after_publish_queued,
            "Queue should decrease or stay same after acknowledgment"
        );
        ...
        assert_eq!(
            after_publish_queued - initial_queued,
            message_count,
            "Should have queued all published messages"
        );
