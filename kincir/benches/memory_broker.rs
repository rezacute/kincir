            ...
            group.bench_with_input(
                ...
                |b, &(count, size)| {
                    b.to_async(&rt).iter(|| async {
                        ...
                        publisher.publish("benchmark_topic", black_box(messages)).await.unwrap();
                    });
                },
            );
        }
...
fn bench_concurrent_operations(c: &mut Criterion) {
    ...
    for publisher_count in [1, 2, 4, 8].iter() {
        for subscriber_count in [1, 2, 4].iter() {
            ...
            group.throughput(Throughput::Elements((publisher_count * messages_per_publisher) as u64));
            group.bench_with_input(
                BenchmarkId::from_parameter(format!("{}pub_{}sub", publisher_count, subscriber_count)),
                    ...
                    b.to_async(&rt).iter(|| async {
                        ...
                        for pub_id in 0..pub_count {
                            ...
                            let handle = tokio::spawn(async move {
                                ...
                                publisher.publish("concurrent_topic", messages).await.unwrap();
                            });
                        ...
                        let messages_per_subscriber = (pub_count * messages_per_publisher) / sub_count;
...
fn bench_message_latency(c: &mut Criterion) {
    ...
    group.bench_function("single_message_latency", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            publisher.publish("latency_topic", vec![black_box(message)]).await.unwrap();
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                b.to_async(&rt).iter(|| async {
                    ...
                    publisher.publish("pattern_topic", black_box(messages)).await.unwrap();
