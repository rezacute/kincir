use kincir::tunnel::{MqttToKafkaTunnel, MqttTunnelConfig, KafkaTunnelConfig};
use std::env;
use dotenvy::dotenv;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> { 
    ...
    let mqtt_broker_url = env::var("MQTT_BROKER_URL").unwrap_or_else(|_| "mqtt://localhost:1883".to_string());
    let mqtt_topics_str = env::var("MQTT_TOPICS").unwrap_or_else(|_| "source/topic,another/source".to_string());
    let mqtt_topics: Vec<String> = mqtt_topics_str.split(',').map(|s| s.trim().to_string()).collect();
    ...
    let kafka_broker_urls_str = env::var("KAFKA_BROKER_URLS").unwrap_or_else(|_| "localhost:9092".to_string());
    let kafka_broker_urls: Vec<String> = kafka_broker_urls_str.split(',').map(|s| s.trim().to_string()).collect();
    ...
    let kafka_config = KafkaTunnelConfig::new(
        kafka_broker_urls,
        &kafka_topic
    );
