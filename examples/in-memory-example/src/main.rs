...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let final_order = vec![Message::new(b"Order #1004: 1x Pizza".to_vec())
    ...
    info!(
        ...
        String::from_utf8_lossy(&order1.payload)
    );
    info!(
        "📨 Subscriber 2 received: {}",
        String::from_utf8_lossy(&order2.payload)
    );
