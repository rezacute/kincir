...
use kincir::ack::{AckConfig, AckHandle, AckStats, AckSubscriber};
use kincir::memory::{InMemoryAckHandle, InMemoryAckSubscriber, InMemoryBroker, InMemoryConfig};
use std::sync::Arc;
...
async fn demonstrate_ack_configs() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let auto_config = AckConfig::auto().with_timeout(Duration::from_secs(10));
    println!("\n📋 Auto ACK Config:");
    ...
    println!(
        "   Dead Letter Topic: {:?}",
        client_auto_config.dead_letter_topic
    );
...
async fn demonstrate_broker_ack_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(stats) = broker.stats() {
        ...
        println!(
            "   Publish Errors (requeues): {}",
            stats
                .publish_errors
                .load(std::sync::atomic::Ordering::Relaxed)
        );
        println!(
            "   Consume Errors (dead letters): {}",
            stats
                .consume_errors
                .load(std::sync::atomic::Ordering::Relaxed)
        );
    ...
    println!(
        "   Subscribed topic: {:?}",
        subscriber.subscribed_topic().await
    );
